...
export class AddPartChapterFormComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private adminActionsService: AdminActionsService) {
    this.adminActionsService.getAllCoursesList().subscribe((values) => {
      ...
      this.adminActionsService.getChaptersFromCourses(this.courseChoosenId).subscribe((vals) => {
  ...
  submit():void {
    ...
    formData.append('text_content', this.partChapterForm.controls.textContent.value);
